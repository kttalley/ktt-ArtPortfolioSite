...
/**
 * Floating artwork card – pristine image + subtle physical depth
 */
function ArtworkCard3D({ artwork, position, index, seed = 0 }) {
    ...
    loader.load(
      ...
      (tex) => {
        ...
        if (imageMaterialRef.current) {
          imageMaterialRef.current.map = tex
  ...
  useFrame((state, delta) => {
    ...
    if (!isDragging) {
      snapBackProgress.current = Math.min(
        1,
        snapBackProgress.current + delta * 2
      )
    ...
    groupRef.current.rotation.x = THREE.MathUtils.lerp(
      ...
      wiggleX + mouse.y * mouseInfluence,
      0.1
    )
    groupRef.current.rotation.y = THREE.MathUtils.lerp(
      groupRef.current.rotation.y,
      wiggleY + mouse.x * mouseInfluence,
      0.1
    )
  ...
  return (
    ...
    >
      ...
      <mesh castShadow receiveShadow position={[0, 0, -0.04]}>
        <boxGeometry args={[cardWidth + 0.06, cardHeight + 0.06, 0.04]} />
